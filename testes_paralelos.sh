...
mede_energia(){
    #$1 Versão a ser executada (1-6) $2 Número de Threads
    sudo scaphandre json -s 1 --max-top-consumers 2 --file consumo.json &
    pid_scaphandre=$!

    ./mult_paralelo m1 m2 $1 $num_thread > mult_time & #Armazena o tempo da multiplicação, executa em segundo plano para obter o pid do processo
    pid_mult=$!
...
#Iniciando os .csv e as pastas
teste(){
    if [ ! -d "$dir" ]; then
    mkdir "$dir"
    fi

    if [ ! -d "$dir/$num_thread" ]; then
        mkdir "$dir/$num_thread"
    fi

    if [ ! -d "$dir/$num_thread/$matdim" ]; then
        mkdir "$dir/$num_thread/$matdim"
    fi

    echo "versao;exec1;exec2;exec3;exec4;exec5" > $dir/$num_thread/$matdim/energias_$matdim.csv
    echo "versao;exec1;exec2;exec3;exec4;exec5" > $dir/$num_thread/$matdim/dramtot_$matdim.csv
    echo "versao;exec1;exec2;exec3;exec4;exec5" > $dir/$num_thread/$matdim/tempos_$matdim.csv
    echo "versao;exec1;exec2;exec3;exec4;exec5" > $dir/$num_thread/$matdim/dram_$matdim.csv
    ...
    for ((i = 1; i<=6; i++)); do
        ...
        for ((j = 1; j<=5;j++)); do
            clear
            echo "Executando versão $i vez $j e $num_thread threads..."
            mede_energia $i
        ...
        echo "$linha_energia" >> $dir/$num_thread/$matdim/energias_$matdim.csv
        echo "$linha_dram_tot" >> $dir/$num_thread/$matdim/dramtot_$matdim.csv
        echo "$linha_tempo" >> $dir/$num_thread/$matdim/tempos_$matdim.csv
        echo "$linha_dram" >> $dir/$num_thread/$matdim/dram_$matdim.csv
    done
}
matdim=$1
dir=$2
